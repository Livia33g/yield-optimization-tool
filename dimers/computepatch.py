...
def calculate_zc(key, energy_fn, all_q0, all_ppos, sigma, kBT, V):
    ...
    debug.print("Result before return: {}", boltzmann_weight * V * (Jtilde/sigma) * zvib)
...
def Calculate_pc_list(N_mon, Zc_monomer, Zc_dimer, exact=False):
    ...
    def Pc(Nd):
        ...
        log_Zc_dimer = jnp.log(Zc_dimer)
        log_Zc_monomer = jnp.log(Zc_monomer)

        # Compute Pc in log-space
        log_pc = jnp.log(Mc(Nd)) + Nd * log_Zc_dimer + 2 * (N_mon_real - Nd) * log_Zc_monomer
        
        # Conditionally normalize if the difference between min and max is too large
        max_log_pc = jnp.max(log_pc)
        min_log_pc = jnp.min(log_pc)

        # Apply conditional normalization only if necessary
        log_pc = jnp.where((max_log_pc - min_log_pc) > 50.0, log_pc - max_log_pc, log_pc)

        # Convert back from log-space to linear space
        val = jnp.exp(log_pc)
        
        # Use lax.cond to handle edge cases for NaN or Inf
        val = lax.cond(jnp.isnan(val) | jnp.isinf(val), 
                       lambda _: 0.0, 
                       lambda _: val, 
                       operand=None)

        debug.print("Value of Pc at Nd {}: {}", Nd, val)
        return val

    # Compute the pc_list using vectorized map
    pc_list = vmap(Pc)(jnp.arange(N_mon_real + 1))

    # Sum the list and use lax.cond to handle NaN or zero sum
    sum_pc_list = jnp.sum(pc_list)
    sum_pc_list = lax.cond(jnp.isnan(sum_pc_list) | (sum_pc_list == 0.0), 
                           lambda _: 1.0, 
                           lambda _: sum_pc_list, 
                           operand=None)

    # Normalize the pc_list
    return pc_list / sum_pc_list
...
def get_argparse():
    ...
    parser.add_argument('-n', '--num-monomer', type=int,  default=25,
...
if __name__ == "__main__":
    ...
    plt.savefig("yields_patch.png")
    plt.clf()

    plt.plot(d0s[:-2], all_grads[:-2])
    plt.savefig("grads_patch.png")
    plt.clf()
