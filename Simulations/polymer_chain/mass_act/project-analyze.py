...
def render(job):
    ...
    with job:
        ...
        fresnel.geometry.Box(scene, box, box_radius=0.07)
...
def polymers_identified(job):
    return (
        job.isfile("polymers/raw_point.png")
        ...
        with gsd.hoomd.open(name="dump.gsd", mode="r") as gsd_file:
            ...
            for i, frame in enumerate(frames):
                ...
                for index in range(frame.particles.N):
                    if (
                        frame.particles.typeid[index] == 0
                        or frame.particles.typeid[index] == 1
                        or frame.particles.typeid[index] == 2
                    ):
                ...
                if i == len(frames) - 1:
                    ...
                    plt.gca().tick_params(
                        axis="both", which="both", labelsize=7, size=4
                    )
                ...
                if i == len(frames) - 1:
                    ...
                    for cluster_id in range(cl.num_clusters):
                        ...
                        cluster_system.plot(
                            ax=ax, s=10, label="Cluster {}".format(cluster_id)
                        )
                ...
                polymer_sizes_by_frame[
                    f"Frame {int(frame.configuration.step / 1e5)}"
                ] = polymer_sizes
            ...
            for key in polymer_sizes_by_frame:
                for subkey in polymer_sizes_by_frame[key]:
                    if subkey in polymer_size_averages.keys():
                        polymer_size_averages[subkey] += np.array(
                            [polymer_sizes_by_frame[key][subkey], 1]
                        )
                    else:
                        polymer_size_averages[subkey] = np.array(
                            [polymer_sizes_by_frame[key][subkey], 1]
            ...
            for key in polymer_size_averages:
                polymer_size_averages[key] = round(
                    polymer_size_averages[key][0] / polymer_size_averages[key][1]
                )
            polymer_sizes_by_frame["Averages"] = dict(
                sorted(
                    polymer_size_averages.items(),
            ...
            rects = ax1.barh(
                pos,
                [
                    polymer_size_averages[key]
                    for key in sorted(
                        polymer_size_averages,
                        key=lambda item: float(item.split(" ")[0]),
                    )
                ],
                align="center",
                height=0.5,
        ...
        def compare(self, cluster2):
            if self.length == cluster2.length:
                ...
                if self.length == 1:
                    ...
                    end_monomers1 = [
                        self.monomer_type_order[0],
                        self.monomer_type_order[-1],
                    ]
                    if end_monomers1 == [
                        cluster2.monomer_type_order[0],
                        cluster2.monomer_type_order[-1],
                    ]:
                        ...
                        return False
                    if (
                        self.monomer_type_order == cluster2_order
                        and self.monomer_headings == cluster2_headings
                    ):
                        return True
                    ...
                    if cyclic_index != -1:
                        ...
                        if (
                            self.monomer_headings
                            == double_cluster2_headings[
                                cyclic_index : cyclic_index + len(cluster2_headings)
                            ]
                        ):
                            return True
            return False
    ...
    with job:
        ...
        with gsd.hoomd.open(name="dump.gsd", mode="r") as gsd_file:
            ...
            for i, frame in enumerate(frames):
                ...
                cl = freud.cluster.Cluster()
                cl.compute(
                    system, neighbors={"mode": "ball", "r_max": cluster_search_radius}
                )
                ...
                for cluster in cl.cluster_keys:
                    ...
                    for monomer_idx in cluster:
                        ...
                        for bond in system.query(
                            system.points,
                            dict(
                                num_neighbors=2,
                                r_max=cluster_search_radius,
                                exclude_ii=True,
                            ),
                        ):
                            ...
                            if bond[2] < 1.5:
                                print(
                                    f"[Warning] Short bond between particles {monomer_idx} and {bond[1]} (length: {bond[2]})"
                                )
                            if bond[0] == monomer_idx and bond[1] in cluster:
                                immediate_neighbors.append(
                                    bond
                                )  # immediate_neighbors.append(bond[1])
                    ...
                    if len(cluster) > 1:
                        try:
                            ...
                            if len(single_neighbor_monomers) == 0:
                                print(
                                    "No single-neighbor monomers found.  Checking for likely circular monomer..."
                                )
                                try:
                                    neighbors = set({})
                                    for monomer_idx in cluster:
                                    ...
                                    raise AssertionError(
                                        "No single-neighbor monomers found, and cluster does not seem to be circular."
                                    )
                                print("Circular polymer likely found.")
                                circular = True
                            else:
                                raise AssertionError(
                                    f"there are {len(single_neighbor_monomers)} monomers with single neighbors, but there should be 0 or 2."
                                )
                        ...
                        if not circular:
                            ...
                            while single_neighbor_monomers[1] not in cluster_order:
                                cluster_order, current_monomer = extend_cluster_order(
                                    cluster_nbrs, cluster_order, current_monomer
                                )
                    ...
                    if len(cluster) > 1:
                        ...
                        monomer1_dir = Rotation.from_quat(
                            monomer1_or, scalar_first=True
                        ).apply([1.0, 0.0, 0.0])
                        monomer2_dir = Rotation.from_quat(
                            monomer2_or, scalar_first=True
                        ).apply([1.0, 0.0, 0.0])
                        dist = np.linalg.norm(monomer2_pos - monomer1_pos)
                        # assert(np.dot(monomer1_dir,monomer2_dir) > 0)
                        if np.linalg.norm(
                            monomer1_pos + monomer1_dir * dist - monomer2_pos
                        ) < np.linalg.norm(
                            monomer1_pos - monomer1_dir * dist - monomer2_pos
                        ):
                    ...
                    cluster_type_order = "".join(
                        typeid_to_type[id]
                        for id in frame.particles.typeid[cluster_order]
                    )
