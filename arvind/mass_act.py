...
init_params = jnp.concatenate([patchy_vals, weak_eps, kT, init_concs])
...
def compute_zvib(energy_fn, q, pos, species, opt_params):
    ...
    zvib = jnp.prod(jnp.sqrt(2.0 * jnp.pi / (opt_params[n] * jnp.abs(evals[6:]) + 1e-12)))
    return zvib
...
def get_log_z_all(opt_params):
    def compute_log_z(size, species, sigma):
        ...
        boltzmann_weight = jnp.exp(-e0 / opt_params[n])
...
def loss_fn(log_concs_struc, log_z_list, opt_params):
    ...
    def log_massact_loss_fn(opt_params, struc_idx):
        ...
        mass_act_loss = -1 / opt_params[n] * e_plus_1 + jnp.sum(presum_mon) + jnp.log(n + 1)
...
def ofer_grad_fn(opt_params, desired_yield_val):
    ...
    def log_massact_loss_fn(opt_params, struc_idx):
        ...
        mass_act_loss = -1 / opt_params[n] * e_plus_1 + jnp.sum(presum_mons) + jnp.log(n + 1)
...
directory_name = "TEMP_STUPID"
# directory_name = "No_Tetramer"
file_name = f"yield_{desired_yield}.txt"
...
with open(output_file_path, "w") as f:
    ...
    f.write(
        f"{args.desired_yield},{final_target_yields},{ mass_act_loss},{params[0]},{params[1]},{params[2]},{params[3]},{params[4]},{params[5]},{params[6]}\n"
    )
    f.flush()
