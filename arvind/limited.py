...
target = jnp.array([0, 2, 3, 0, 4, 5, 0])
...
for i in range(1, num_monomers + 1):
    ...
    tot_num_structures += len(species_data[key])
...
def indx_of_target(target, species_data):
    ...
    for i in range(1, num_monomers + 1):
        ...
        for j in range(len(current_species)): # Corrected this line previously
            if jnp.array_equal(current_species[j], target) or jnp.array_equal(
                current_species[j], target_reversed
            ):
                return j + offset
        offset += len(current_species) # Corrected this line now!
...
def make_shape(species_sequence):
    # Define monomer templates
    monomer_templates = {
        (0, 2): jnp.array(
            [
                [
                ...
                [
                    a,
                    b * jnp.cos(jnp.pi / 6.0),
                    -b * jnp.sin(jnp.pi / 6.0),
                ],  # second patch
                [
                    a,
                    -b * jnp.cos(jnp.pi / 6.0),
    ...
    # Check if species sequence is valid
    monomer_shapes = []
    i = 0
    while i < len(species_sequence):
        found = False
        for key in monomer_templates:
            if species_sequence[i : i + len(key)] == list(key):
                monomer_shapes.append(monomer_templates[key])
                i += len(key)
                found = True
                break
        if not found:
            raise ValueError(
                f"Invalid species sequence at position {i}: {species_sequence}"
            )

    return jnp.concatenate(monomer_shapes, axis=0)
...
def get_nmer_energy_fn(n):
    """
    Computes the energy of an n-mer while being fully JIT-compatible.
    - Uses `jax.lax.scan` for slicing dynamically.
    - Uses JAX indexing instead of Python loops.
    - Fully vectorized with `vmap` for efficiency.
    """

    def nmer_energy_fn(q, pos, species, opt_params):
        positions = utils.get_positions(q, pos)

        num_patches_per_monomer = jnp.array([len(sp) for sp in species])
        num_vertices_per_monomer = num_patches_per_monomer  # Each patch has a vertex
        pos_start_indices = 3 * jnp.cumsum(num_vertices_per_monomer) - (
            3 * num_vertices_per_monomer
        )
        species_start_indices = (
            jnp.cumsum(num_patches_per_monomer) - num_patches_per_monomer
        )

        def slice_fn(carry, indices):
            pos_start, num_verts, species_start, num_patches = indices
            pos_end = pos_start + num_verts * 3
            species_end = species_start + num_patches

            monomer_pos = positions[pos_start:pos_end]
            monomer_species = species[species_start:species_end]

            return carry, (monomer_pos, monomer_species)

        _, (all_pos, all_species) = lax.scan(
            slice_fn,
            None,
            (
                pos_start_indices,
                num_vertices_per_monomer,
                species_start_indices,
                num_patches_per_monomer,
            ),
        )

        all_species = jnp.concatenate([jnp.repeat(sp, 3) for sp in all_species])
        ...
        pairs = jnp.array(list(itertools.combinations(jnp.arange(n), 2)))
        all_pairwise_energies = vmap(pairwise_energy)(pairs)

        return all_pairwise_energies.sum()

    return jit(nmer_energy_fn)


def hess(energy_fn, q, pos, species, opt_params):
    H = hessian(energy_fn)(q, pos, species, opt_params)
    evals, evecs = jnp.linalg.eigh(H)
    print(f"Shape of evals in hess: {evals.shape}") # ADDED PRINT
    print(f"Shape of evecs in hess: {evecs.shape}") # ADDED PRINT
    return evals, evecs
...
def compute_zrot_mod_sigma(energy_fn, q, pos, species, opt_params, key, nrandom=10000):
    ...
    def ftilde(nu):
        ...
        print(f"Shape of evecs.T[6:].T in ftilde: {evecs.T[6:].T.shape}") # ADDED PRINT
        print(f"Shape of nu[6:] in ftilde: {nu[6:].shape}") # ADDED PRINT
        q_tilde = jnp.matmul(evecs.T[6:].T, nu[6:]) # Error line
...
energy_fns = {size: get_nmer_energy_fn(size) for size in range(2, num_monomers + 1)}


rb1 = rbs[1]
shapeA = make_shape([0, 2])
shapeB = make_shape([3, 0, 4])
shapeC = make_shape([5, 0])
# sigma1 = data["1_sigma"]
mon_energy_fn = lambda q, pos, species, opt_params: 0.0


zrot_mod_sigma_A, _, main_key = compute_zrot_mod_sigma(
    mon_energy_fn, rb1, shapeA, jnp.array([0, 2]), patchy_vals, main_key
)
zrot_mod_sigma_B, _, main_key = compute_zrot_mod_sigma(
    mon_energy_fn, rb1, shapeB, jnp.array([3, 0, 4]), patchy_vals, main_key
)
zrot_mod_sigma_C, _, main_key = compute_zrot_mod_sigma(
    mon_energy_fn, rb1, shapeC, jnp.array([5, 0]), patchy_vals, main_key
)
zvib_1 = 1.0
boltzmann_weight = 1.0

z_A = compute_zc(boltzmann_weight, zrot_mod_sigma_A, zvib_1, sigmas[1])
z_B = compute_zc(boltzmann_weight, zrot_mod_sigma_B, zvib_1, sigmas[1])
z_C = compute_zc(boltzmann_weight, zrot_mod_sigma_C, zvib_1, sigmas[1])

z_1s = jnp.array([z_A, z_B, z_C])
log_z_1 = jnp.log(z_1s)

shapes = {}

for size in range(2, num_monomers + 1):
    species_list = species_data[f"{size}_pc_species"]

    for i in range(len(species_list)):
        species_tuple = tuple(
            species_list[i]
        )  # Convert species array to tuple for dict key

        if (size, species_tuple) not in shapes:  # Only compute once
            shapes[(size, species_tuple)] = make_shape(
                species_list[i]
            )  # Compute shape based on species
...
for size in range(2, num_monomers + 1):
    ...
    for i in range(len(species_list)):
        species_tuple = tuple(
            species_list[i]
        )  # Convert species array to tuple for lookup

        if (size, species_tuple) not in zrot_mod_sigma_values:  # Avoid recomputation
            shape = shapes[(size, species_tuple)]  # Get species-specific shape

            zrot_mod_sigma, Js, main_key = compute_zrot_mod_sigma(
                energy_fns[size],
                rbs[size],
                shape,  # Use species-dependent shape
                species_list[i],  # Pass species array
                patchy_vals,
                main_key,
            )
            zrot_mod_sigma_values[(size, species_tuple)] = (
                zrot_mod_sigma  # Store per species
            )
...
def get_log_z_all(opt_params):
    def compute_log_z(size, species, sigma):
        ...
        shape = shapes[(size, species_tuple)]
        zrot_mod_sigma = zrot_mod_sigma_values[(size, species_tuple)]

        # Compute partition function components
    ...
    for size in range(2, num_monomers + 1):
        species = species_data[f"{size}_pc_species"]
        sigma = data[f"{size}_sigma"]

        sigma_array = jnp.full(len(species), sigma)
    ...
    print(f"Shape of log_z_all (using len): {len(log_z_all)}") 
...
with open(output_file_path, "w") as f:
    ...
    f.write(
        f"{args.desired_yield},{final_target_yields},{kT},{params[0]},{params[1]},{params[2]},{params[3]},{params[4]},{params[5]}\n"
    )
    f.flush()
