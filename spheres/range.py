...
parser.add_argument("--kt_low", type=float, default=0.2, help="Initial temperature (kbT)")
parser.add_argument("--kt_high", type=float, default=1.5, help="Initial temperature (kbT)")
parser.add_argument(
    "--init_morse", type=float, default=7.0, help="Initial Morse epsilon"
)
parser.add_argument(
    "--rep_alpha", type=float, default=1., help="Initial repulsion alpha"
)
parser.add_argument(
...
parser.add_argument(
    "--desired_yield_l", type=float, default=1., help="Desired yield for low temperature"
)
...
init_params = jnp.array([args.init_morse, 2.0, 10000.0, args.rep_alpha, args.kt_high, args.kt_low])
# --- JAX-MD Setup ---
...
def compute_zvib(energy_fn, q, pos, species, opt_params):
    ...
    zvib_low = jnp.prod( jnp.sqrt(2.0 * jnp.pi / ((opt_params[-1]) * (jnp.abs(evals[6:]) + 1e-12)))
    )
...
def ofer_grad_fn(opt_params, desired_yield_val_h, desired_yield_val_l):
    ...
    loss_h = (1.0 - jnp.exp(log_yield_h)) ** 2 + 0.1 * (log_yield_h - target_h) ** 2 + 0.05 * energy_penalty
    loss_l = (1.0 - jnp.exp(log_yield_l)) ** 2 + 0.1 * (log_yield_l - target_l) ** 2 + 0.05 * energy_penalty
    total_loss = loss_h + loss_l
    return  total_loss
...
our_grad_fn = jit(value_and_grad(lambda p, yh, yl: (yh - jnp.exp(ofer(p)[0])) ** 2 + (yl - jnp.exp(ofer(p)[1])) ** 2 ))
params = init_params
...
params = init_params
opt_state = outer_optimizer.init(params)
with open(f"TEST/test/{kt_low}_{kt_high}.txt", "w") as f:
    for i in range(n_outer_iters):
        params, opt_state, loss = outer_step(params, opt_state, desired_yield_val_h, desired_yield_val_l)
    ...
    f.write(f"{final_target_yields_h}, {final_target_yields_l},{params[0]},{params[-2]},{params[-1]}\n")
