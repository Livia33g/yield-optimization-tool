...
pdb.set_trace()
"""
energy_fns = {size: jit(get_nmer_energy_fn(size)) for size in list_size[1:]}
...
def get_log_z_all(opt_params):
    def compute_log_z(size):
        ...
        # print(f"rb: {rb}")
        specie = species[size]
        sigma = sigmas[en_size]
        zrot_mod_sigma = zrot_mod_sigma_values[size - 1]
        zvib = compute_zvib(energy_fn, rb, shape, specie, opt_params)
        e0 = energy_fn(rb, shape, species, opt_params)
        boltzmann_weight = jnp.exp(-e0 / opt_params[-1])
...
def loss_fn(log_concs_struc, log_z_list):
    ...
    struc_loss = vmap(struc_loss_fn)(jnp.arange(1, 3))
